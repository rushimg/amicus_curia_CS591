...
K = 10
OUT_FILE = "clust_conv_"+ str(K) +".csv"
...
def k_means(in_dict,bills_govtrack,orgs):
	...
	j = 0
	CONV = False
	centroids_dict_old = centroids_dict
	while not(CONV):
		j += 1
		...
		if (float(len(set(centroids_dict.values()).intersection(set(centroids_dict_old.values()))))/float(len(set(centroids_dict.values()).union(set(centroids_dict_old.values()))))==1) and j > 5:
			print centroids_dict_old
			print centroids_dict
			CONV = True
		centroids_dict_old = centroids_dict	
	#print centroids_dict
